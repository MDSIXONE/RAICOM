...
rows = []
for result in model.predict(source=[str(p) for p in paths], conf=0.12, iou=0.5, imgsz=640, device=0, verbose=False, stream=True):
    dets = []
    if result.boxes is not None:
        for xyxy, cls, conf in zip(result.boxes.xyxy.tolist(), result.boxes.cls.tolist(), result.boxes.conf.tolist()):
            dets.append({"cls": int(cls), "conf": float(conf), "xyxy": [float(v) for v in xyxy]})
...
(OUT / "model_candidates.json").write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")
