...
os.environ['PATH'] = '../xgovenv/bin:' + os.environ.get('PATH', '')
os.chdir('..')  
...
from uiutils import (Button, splash_theme_color, btn_selected, btn_unselected, txt_selected, 
...
# Load Images
BASE_DIR = "/home/pi/RaspberryPi-CM5/common"

app_image = Image.open(os.path.join(BASE_DIR, "pics/riderapp.png")) 
unapp_image = Image.open(os.path.join(BASE_DIR, "pics/riderunapp.png"))
wifiy = Image.open(os.path.join(BASE_DIR, "pics/wifi@2x.jpg"))
wifin = Image.open(os.path.join(BASE_DIR, "pics/wifi-un@2x.jpg"))
font2 = ImageFont.truetype(os.path.join(BASE_DIR, "model/msyh.ttc"), 23)
...
def handle_left(data):  
    global last_access_time
    last_access_time = time.time()
    executor.submit(execute_action, dog.rider_turn, int(data))
...
def handle_height(data):
    ...
    if data < 50:
        height = int(95 - (50 - data) * 1.0)  
    else:
        height = int(95 + (data - 50) * 1.0)  
