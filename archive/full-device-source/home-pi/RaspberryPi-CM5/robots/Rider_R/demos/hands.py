...
ACTIONS = {"one": 1, "two": 2, "three": 3, "four": 4, "five": 5, "six": 6}
COOLDOWN_SECONDS = 3
MIN_STABLE_FRAMES = 4
await_release = False
...
def main():
    while True:
        ...
        # Debounce: count stable frames for action gestures
        current_gesture = gesture_result if gesture_result else "not in detect range..."

        # Local static variables for stability tracking
        if not hasattr(main, "_prev_gesture"):
            main._prev_gesture = None
            main._stable_count = 0

        if current_gesture == main._prev_gesture:
            main._stable_count += 1
        else:
            main._prev_gesture = current_gesture
            main._stable_count = 1

        if current_gesture in ACTIONS:
            if main._stable_count >= MIN_STABLE_FRAMES:
                process_gesture(current_gesture)
        else:
            # release immediately for non-action gestures
            process_gesture(current_gesture)
