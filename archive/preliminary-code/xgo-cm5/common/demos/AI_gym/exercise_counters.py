...
class ExerciseCounter:
    ...
    def __init__(self):
        self.counter = 0
        self.stage = None
        self.ding_sound_path = "/home/pi/RaspberryPi-CM5/common/music/dingding.wav"
        
        # 添加路径验证
        print(f"声音文件路径: {self.ding_sound_path}")
        print(f"文件是否存在: {os.path.exists(self.ding_sound_path)}")
        if os.path.exists(self.ding_sound_path):
            print(f"文件大小: {os.path.getsize(self.ding_sound_path)} 字节")
        else:
    ...
    def play_ding_sound(self):
        ...
        try:
            ...
            if not pygame.mixer.get_init():
                print("Mixer 未初始化，尝试重新初始化")
                pygame.mixer.init()
            
            if self.ding_sound_path and os.path.exists(self.ding_sound_path):
                print("播放声音...")
                sound = pygame.mixer.Sound(self.ding_sound_path)
                channel = sound.play()
    ...
    def count_pushup(self, keypoints):
        ...
        try:
            ...
            if left_angle > 160 and right_angle > 160:
                self.stage = "up"
            elif left_angle < 90 and right_angle < 90 and self.stage == "up":
                self.stage = "down"
                self.counter += 1
                self.play_ding_sound()
    ...
    def get_angle_point(self, keypoints, exercise_type):
        """根据运动类型获取角度显示位置"""
        try:
            if exercise_type == "squat": #深蹲
                # 膝盖关节点 (右膝) - 14
                return (int(keypoints[14][0]), int(keypoints[14][1]))
                
            elif exercise_type == "pushup": #俯卧撑
                # 肘部关节点 (右肘) - 8
