#!/usr/bin/env bash
# Interactive continuous-control launcher. The helper gives the OUMAX service
...
docker exec -it ros-noetic bash -lc '
  ...
  exec roslaunch robot_dog_teleop physical_keyboard_continuous.launch enable_motion:=true
'
