...
namespace cym_planner
{
    ...
    void CymPlanner::initialize(std::string name, tf2_ros::Buffer* tf, costmap_2d::Costmap2DROS* costmap_ros)
    {
        ...
        planner_nh.param<std::string>("map_image_topic", map_image_topic_,
                                      "/cym_planner/map_image");
        planner_nh.param<std::string>("plan_image_topic", plan_image_topic_,
                                      "/cym_planner/plan_image");
        map_image_pub_ = public_nh.advertise<sensor_msgs::Image>(map_image_topic_, 1);
        plan_image_pub_ = public_nh.advertise<sensor_msgs::Image>(plan_image_topic_, 1);
    ...
    bool CymPlanner::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
    {
        ...
        //publish costmap image to RViz
        cv::resize(map_image, map_image, cv::Size(size_x*5, size_y*5),0,0,cv::INTER_NEAREST);
        publishImage(map_image, costmap_frame, map_image_pub_);
        ...
        //publish plan image to RViz
        publishImage(plane_image, base_link_frame_, plan_image_pub_);
        return true;
    }
    ...
    void CymPlanner::publishImage(const cv::Mat& image, const std::string& frame_id,
